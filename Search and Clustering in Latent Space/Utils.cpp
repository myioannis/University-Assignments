...
double manhattan_Distance(Image<T>* image1, Image<T>* image2)
{
    ...
    double sum = 0;
    vector<T> pixels1 = image1->get_pixels();
    vector<T> pixels2 = image2->get_pixels();
...
{  
    int leftmost_byte, rightmost_byte, result;
    leftmost_byte = (value & 0x000000FF) >> 0;
    rightmost_byte = (value & 0x0000FF00) >> 8;
    leftmost_byte <<= 8;
    rightmost_byte <<= 0;
    value = (leftmost_byte | rightmost_byte);
}

template double manhattan_Distance<unsigned char>(Image<unsigned char>* image1, Image<unsigned char>* image2);
template double manhattan_Distance<unsigned short>(Image<unsigned short>* image1, Image<unsigned short>* image2);

template void swap_Endians_Reduced<unsigned char>(unsigned char& value);
template void swap_Endians_Reduced<unsigned short>(unsigned short& value);